...
Phaser.Plugin.Juicy.prototype.jelly = function(object, strength, delay, initialScale) {
  ...
  var xTween = this.game.add.tween(object.scale).to({x: initialScale.x + (initialScale.x * strength)}, 50, Phaser.Easing.Quadratic.InOut, false, delay);
  xTween.to({x: initialScale.x}, 600, Phaser.Easing.Elastic.Out, false);
  xTween.start();

  var yTween = this.game.add.tween(object.scale).to({y: initialScale.y + (initialScale.y * strength)}, 50, Phaser.Easing.Quadratic.InOut, false, delay + 50)
  yTween.to({y: initialScale.y}, 600, Phaser.Easing.Elastic.Out, false);
  yTween.start();
};
